...
That first repair unified the navigation. SAVE-01 and SAVE-02 now add account persistence behind it: website lists and asset favourites use separate typed snapshots, guest data migrates safely, and revision conflicts preserve explicit add/remove operations. A future combined All view should retain explicit Website/Asset labels and source/framework information rather than pretending a provider and one of its icons are the same record.

## Completed work packages

- **SAVE-01 — safe website-list sync:** completed on the integration branch. Remote baselines and optimistic revisions now guard writes; failed reads/writes retain local operations and expose retry. See the [implementation log](IMPLEMENTATION-LOG-2026-09-12.md).
- **SAVE-02 — typed account-backed asset favourites:** completed on the integration branch. Guest migration, account isolation, cross-tab state and cross-device conflict replay are covered. The additive Neon migration has been applied and inspected. Live preview acceptance is recorded in the implementation log.
- **MEDIA-01 — representative real component rendering:** completed on the integration branch. Five shadcn/ui primitives are retained from one immutable commit, verified against upstream hashes and rendered through a reviewed allow-list; all other components keep the explicit illustration fallback. Desktop and narrow-viewport gallery/detail acceptance is recorded in the implementation log.

## Prioritised work register

| Priority | Gap or defect                                        | Evidence / current state                                                                                                                                                                               | Completion condition                                                                                                                                       |
| -------- | ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Done     | Asset saves do not sync to accounts                  | Completed as SAVE-02; typed endpoint, guest migration, owner isolation and revision conflict tests are recorded in the implementation log                                                              | Typed website/asset saved items, owner-scoped API, guest migration, cross-device save/remove tests; retain existing saves                                  |
| Done     | Website save failure handling can lose updates       | Completed as SAVE-01; writes wait for a baseline and queued operations are replayed over revision conflicts                                                                                            | Do not enable snapshot writes until baseline is loaded, preserve pending changes, show error/retry and reconcile concurrent edits                          |
| Done     | Representative real component previews               | MEDIA-01 retains and integrity-checks five shadcn/ui primitives from immutable commit `2b3e6d4`; arbitrary remote code cannot enter the renderer and unsupported records remain labelled illustrations | Reviewed allow-list, retained licence, deterministic gallery/detail renders, desktop/mobile review and honest fallback                                     |
