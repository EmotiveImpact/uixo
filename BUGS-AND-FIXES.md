...
**Found 2026-09-14 during pre-release browser checks:** Setting a provider `<base>` redirected Vite's lazy CSS URLs to Simply Buttons instead of UIXO, so the first 20 demonstrations failed to start despite compiling successfully.
...
## A provider subset was presented as the catalogue

**Found 2026-09-14:** The initial Simply Buttons import stopped after 20 eligible entries even though the pinned source catalogue contained 108. That limit was an implementation shortcut, not a source or product limit.

**Fix:** The retained snapshot now contains all 108 unique source entries. Its build guard requires exactly 108 matching live demos, including local dependency closure and declared third-party packages. Browser verification must reach `ready` for every entry before publication. Magic UI is 68/68 source-backed entries because seven registry records have no source file at its pinned revision; Motion Primitives is 33/33.
