...
export async function handleMcp(
  ...
  try {
    ...
    if (!req.headers.host || !hosts.has(req.headers.host))
      throw new RegistryError('HOST_BLOCKED', 'Host is not allowed.', 403);
    ...
    if (process.env.UIXO_MCP_REQUIRE_TOKEN === 'true') {
      const supplied = req.headers.authorization?.startsWith('Bearer ')
        ...
        : '';
      if (!tokenMatches(supplied, process.env.UIXO_MCP_TOKEN)) {
        res.setHeader('www-authenticate', 'Bearer realm="uixo"');
        throw new RegistryError('UNAUTHORISED', 'A valid UIXO MCP service token is required.', 401);
      }
    }
    ...
    const response = await handler.fetch(
      new Request(`${origin}/api/mcp`, { method: req.method ?? 'POST', headers, body }),
    );
    ...
    if (!res.headersSent)
      json(res, error instanceof RegistryError ? error.status : 503, {
        error: {
          code: error instanceof RegistryError ? error.code : 'MCP_UNAVAILABLE',
          message:
            error instanceof RegistryError
              ? error.message
              : 'MCP is unavailable. Check its installed dependencies and registry configuration.',
        },
      });
    else res.end();
  } finally {
