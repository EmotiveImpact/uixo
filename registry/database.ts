...
export async function sqliteDatabase(path = ':memory:', snapshot = false): Promise<Database> {
  ...
  const run = (sql: string, args: (string | number | null)[] = []): Row[] => {
    ...
    const translated = sql.replace(/\$(\d+)/g, (_, n: string) => { ordered.push(args[Number(n) - 1]); return '?'; });
    return db.prepare(translated).all(...ordered) as Row[];
  };
  return {
    mode: snapshot ? 'snapshot' : 'sqlite',
    async query(sql, args) { return run(sql, args); },
    async batch(statements) {
      db.exec('BEGIN IMMEDIATE');
      try { const result = statements.map((s) => run(s.sql, s.args)); db.exec('COMMIT'); return result; }
...
export async function migrate(db: Database) {
  const source = await readFile(new URL('../db/migrations/002-registry.sql', import.meta.url), 'utf8');
  const statements = source.split(';').map((sql) => sql.trim()).filter(Boolean).map((sql) => ({ sql }));
