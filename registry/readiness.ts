...
export async function intelligenceReadiness(registry: Registry) {
  ...
  const sql =
    registry.db.mode === 'postgres'
      ? 'SELECT table_name AS name FROM information_schema.tables WHERE table_schema=current_schema() AND table_name IN (' +
        placeholders +
        ')'
  ...
  return {
    ...
    mode:
      registry.db.mode === 'snapshot' ? ('read-only-evaluation' as const) : ('persistent' as const),
  };
}
