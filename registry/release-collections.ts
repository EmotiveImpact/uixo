...
export async function bootstrapSnapshotCollections(registry: Registry) {
  if (registry.db.mode !== 'snapshot')
    throw new RegistryError('SNAPSHOT_ONLY', 'Snapshot editorial bootstrap cannot target persistent storage.', 403);
  let published = 0;
  for (const collection of STARTER_COLLECTIONS) {
    if ((await registry.db.query('SELECT slug FROM uixo_v2_collections WHERE slug=$1', [collection.slug])).length)
      continue;
    ...
    collection.items.forEach((item, position) => statements.push({
    ...
    statements.push({
      sql: 'INSERT INTO uixo_v2_audit(id,actor,action,target,detail,created_at) SELECT $3,$4,$5,$1,$6,$7 WHERE ' + guard,
      args: [collection.slug, token, randomUUID(), actor, 'collection-publish', JSON.stringify({
        reason: 'Release-authored source-backed selection for the read-only evaluation catalogue.',
 ...
 */
export async function publishNewStarterCollections(registry: Registry, actor: string, reason: string) {
  if (registry.db.mode === 'snapshot')
    throw new RegistryError('PERSISTENT_REQUIRED', 'Use persistent storage for operator publication.', 403);
  if (!actor.trim() || reason.trim().length < 10)
    throw new RegistryError('INVALID_INPUT', 'An operator identity and a meaningful review reason are required.');
  let published = 0;
