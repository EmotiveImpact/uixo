...
export async function readJson(req: RequestLike, limit = 200_000): Promise<unknown> {
  if (!req.headers['content-type']?.startsWith('application/json'))
    throw new RegistryError('CONTENT_TYPE', 'Use application/json.', 415);
  if (Number(req.headers['content-length'] ?? 0) > limit)
    throw new RegistryError('BODY_TOO_LARGE', 'Request body is too large.', 413);
  if (req.body !== undefined) {
    ...
    try {
      ...
      throw new RegistryError('INVALID_JSON', 'Request body must be valid JSON.');
    }
  }
  const chunks: Buffer[] = [];
  let bytes = 0;
  for await (const chunk of req) {
    ...
    bytes += buffer.length;
    if (bytes > limit) throw new RegistryError('BODY_TOO_LARGE', 'Request body is too large.', 413);
    chunks.push(buffer);
  }
  try {
...
export function checkOrigin(req: IncomingMessage, expectedOrigin: string) {
  ...
  if (origin && origin !== expectedOrigin)
    throw new RegistryError('ORIGIN_BLOCKED', 'Cross-origin requests are not allowed.', 403);
}
export async function rateLimit(registry: Registry, key: string, maximum = 180) {
  const minute = Math.floor(Date.now() / 60000);
  const bucket = createHash('sha256')
    .update(`${process.env.UIXO_RATE_SALT ?? 'uixo'}:${key}:${minute}`)
    .digest('hex');
  const expiry = new Date((minute + 2) * 60000).toISOString();
  const rows = await registry.db.query(
    'INSERT INTO uixo_v2_rate_limits(bucket,count,expires_at) VALUES($1,1,$2) ON CONFLICT(bucket) DO UPDATE SET count=uixo_v2_rate_limits.count+1 RETURNING count',
    [bucket, expiry],
  );
  if (Number(rows[0].count) > maximum)
    throw new RegistryError('RATE_LIMITED', 'Too many requests. Try again shortly.', 429);
  if (Math.random() < 0.02)
    await registry.db.query('DELETE FROM uixo_v2_rate_limits WHERE expires_at<$1', [
      new Date().toISOString(),
    ]);
}
export function createRegistryHandler(
  registry: Registry,
  options: { origin: string; authenticate?: (req: RequestLike) => Promise<Principal> },
) {
  ...
  return async (req: RequestLike, res: ServerResponse) => {
    ...
    try {
      ...
      if (!routes[action].includes(method)) {
        res.setHeader('allow', routes[action].join(', '));
        throw new RegistryError('METHOD_NOT_ALLOWED', 'Method not allowed.', 405);
      }
      const ip = process.env.VERCEL
        ? String(
            req.headers['x-vercel-forwarded-for'] ?? req.headers['x-forwarded-for'] ?? 'unknown',
          )
            .split(',')[0]
            .trim()
      ...
      if (
        ...
        throw new RegistryError(
          'UNAUTHORISED',
          'A curator account or an appropriately scoped service credential is required.',
          401,
        );
      if (operatorAction && method === 'POST' && registry.db.mode === 'snapshot')
        throw new RegistryError(
          'READ_ONLY_SNAPSHOT',
          'This deployment is a read-only source snapshot. Configure a persistent registry database to enable writes.',
          503,
        );
      ...
      if (action === 'search') {
        ...
        result = {
          ...
          role: who?.role ?? 'visitor',
          eveConfigured: Boolean(process.env.UIXO_EVE_URL && process.env.UIXO_EVE_TOKEN),
          searchMode: 'weighted-keyword',
          mcpPath: '/api/mcp',
        };
      } else if (action === 'queue')
        result = await registry.queue(
          integer(url.searchParams.get('limit') ?? undefined, 48, 1, 48),
          integer(url.searchParams.get('offset') ?? undefined, 0, 0, 100000),
        );
      else if (action === 'review') {
        const decision = text(body.decision, 20);
        if (!['approve', 'reject'].includes(decision))
          throw new RegistryError('INVALID_INPUT', 'Decision must be approve or reject.');
        const reason = text(body.reason, 2000);
        if (reason.length < 10)
          throw new RegistryError(
            'INVALID_INPUT',
            'Give a review reason of at least 10 characters.',
          );
        result = await registry.review(
          identifier(body.id),
        ...
        result =
          method === 'POST' ? await registry.scout(body) : { items: await registry.scoutQueue() };
      else if (action === 'jobs') result = { items: await jobs(registry) };
      else if (action === 'enqueue')
        result = await enqueue(
          registry,
          identifier(body.providerId),
          body.afterJobId ? identifier(body.afterJobId) : undefined,
        );
      else if (action === 'run') result = await runJob(registry, identifier(body.id));
      else {
        const asset = await registry.inspect(identifier(body.id));
        const variant = body.variantId ? identifier(body.variantId) : undefined;
        result =
          action === 'resolve'
            ? resolveAsset(asset, variant)
            : checkCompatibility(asset, body.project ?? {}, variant);
      }
      json(res, 200, result);
    } catch (error) {
      ...
      if (!known)
        console.error(
          JSON.stringify({ event: 'registry_request_failed', requestId, error: 'INTERNAL_ERROR' }),
        );
      if (!res.headersSent)
        json(res, known ? error.status : 503, {
          error: {
            ...
            message: known
              ? error.message
              : 'The registry is unavailable. Check its database configuration and migration status.',
            requestId,
          },
        });
      else res.end();
